...
// Bahnhöfe für Filter sammeln (Sowohl aus Start- als auch Zielbahnhöfen)
...
// Basis-Query: Holt NUR die jeweils aktuellste Version jeder incident_id
$query_str = "
...
<head>
    ...
    <style>
        ...
        /* Farbvariationen nach Vorgabe */
        .type-info { background: #1a2a6c; }        /* Blau: Vorinformation */
        .type-construction { background: #d96b00; }/* Orange: Bauarbeiten */
        .type-disruption { background: #d30000; }  /* Rot: Störung */
...
<div class="filter-panel">
    <form method="GET" action="index.php" style="display: flex; flex-wrap: wrap; gap: 15px; align-items: flex-end; width: 100%;">
        <div>
            <label for="station">Bahnhof filtern:</label>
            <select id="station" name="station">
                <option value="">-- Alle Betriebsstellen --</option>
        ...
        <div>
            ...
            <select id="time_filter" name="time_filter" onchange="this.form.submit()">
                <option value="current" <?php if($time_filter === 'current') echo 'selected'; ?>>Aktuell aktive Meldungen</option>
                <option value="all" <?php if($time_filter === 'all') echo 'selected'; ?>>Gesamtes Jahr / Archiv</option>
                <option value="custom" <?php if($time_filter === 'custom') echo 'selected'; ?>>Individueller Zeitraum...</option>
            </select>
        </div>
